    ...
    <script>
        ...
        const showSavedCheckmark = (footer) => {
            ...
            checkmark.innerHTML = `
                    <span class="saved-indicator d-flex align-items-center gap-1">
                        ...
                        <span class="text-primary fw-bold fs-7">Saved</span>
                    </span>
                `;
        ...
        const appendMessage = (role, content, tokens = 0, messageId = null, hidden = false) => {
            ...
            const configSelect = document.getElementById(
                'quick-chat-model-selector-{{ $session?->id ?? 'default' }}');
            ...
            const template = document.getElementById(
                'message-bubble-template-{{ $session?->id ?? 'default' }}');
            if (!template) {
            ...
            if (role === 'assistant' && provider && model) {
                ...
                headerText.textContent = role === 'user' ? '{{ auth()->user()->name ?? 'User' }}' :
                    'Assistant';
            }
            ...
            if (contentDiv) {
                contentDiv.querySelectorAll('pre').forEach(pre => {
                    if (!pre.querySelector('.copy-code-btn')) {
                        ...
                        copyBtn.className =
                            'btn btn-icon btn-sm btn-light-primary position-absolute top-0 end-0 m-1 copy-code-btn';
                        ...
                        copyBtn.innerHTML =
                            '<i class="ki-duotone ki-copy fs-7"><span class="path1"></span><span class="path2"></span></i>';
            ...
            // BUG-3 fix: Copy button for ALL roles, Raw button only for assistant
            const bubbleContent = messagesContainer.querySelector(
                `[data-message-id="${messageId}"] .bubble-content-wrapper`);
            if (bubbleContent && !bubbleContent.querySelector('.message-actions-container')) {
                const btnContainer = document.createElement('div');
                btnContainer.className =
                    'message-actions-container position-absolute top-0 end-0 m-2 d-flex gap-1';

                // Raw view button (only for assistant AND if message ID exists and is NOT temporary)
                if (role === 'assistant' && messageId && !messageId.toString().startsWith('msg-')) {
                ...
                copyBtn.onclick = function() {
                    copyBubbleContent(this);
                };
                copyBtn.innerHTML =
                    '<i class="ki-duotone ki-copy fs-6"><span class="path1"></span><span class="path2"></span></i>';
        ...
        const sendMessage = async () => {
            ...
            addMonitorLog(
                `   Prompt: "${userPrompt.substring(0, 50)}${userPrompt.length > 50 ? '...' : ''}"`,
                'debug');
            ...
            const thinkingModelInfo = document.getElementById(
                'thinking-model-info-{{ $session?->id ?? 'default' }}');
            ...
            eventSource.addEventListener('request_data', (event) => {
                ...
                console.log(
                    '[Event Handlers] SSE request_data received (COMPLETE with context)',
                    data);
                ...
                if (typeof window.populateRequestInspector === 'function') {
                    ...
                    addMonitorLog('📋 Request Inspector: updated with context_messages',
                        'success');
                    console.log(
                        '[Event Handlers] Request Inspector updated with context_messages');
                } else {
                    console.warn(
                        '[Event Handlers] populateRequestInspector function not found');
                }
            });
            ...
            const baseMaxTokens =
                {{ $configurations->first()->default_parameters['max_tokens'] ?? 8000 }};
            ...
            statsUpdateInterval = setInterval(() => {
                if (thinkingMessage && !thinkingMessage.classList.contains('d-none')) {
                    ...
                    const ttft = firstChunkTime ? ((firstChunkTime - startTime) / 1000).toFixed(
                        2) : '...';
            ...
            eventSource.onmessage = (event) => {
                ...
                if (data.type === 'metadata') {
                    ...
                    if (chunkCount === 1) {
                        const assistantBubble = messagesContainer.querySelector(
                    ...
                    if (chunkCount % 50 === 0) {
                        addMonitorLog(`📥 Received ${chunkCount} chunks (${currentTokens} tokens)`,
                            'info');
                    }
                    ...
                    if (currentTokens > threshold && !warningShown) {
                        ...
                        addMonitorLog(
                            `   Usage: ${Math.round((currentTokens/currentMaxTokens)*100)}%`,
                            'debug');
                        addMonitorLog('', 'info');
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                warningShown = false;
                                addMonitorLog(
                                    `✅ Token limit extended to ${currentMaxTokens}`,
                                    'success');
                                addMonitorLog(
                                    `   Next warning at ${Math.round(currentMaxTokens * 0.9)} tokens (90%)`,
                                    'debug');
                                addMonitorLog('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
                                    'separator');
                                addMonitorLog('', 'info');
                    ...
                    if (data.message_id) {
                        ...
                        if (assistantBubble) {
                            ...
                            if (!isAtBottom()) {
                                updateUnreadBadge(1);
                                console.log(
                                    '[Unread Badge] Assistant message completed, user not at bottom'
                                );
                            }

                            // Add error badge if this is an error message
                            if (data.metadata?.is_error) {
                                const headerDiv = assistantBubble.querySelector('.text-gray-600')
                                    ?.parentElement;
                                if (headerDiv && !headerDiv.querySelector('.badge-warning')) {
                                    const errorBadge = document.createElement('span');
                                    errorBadge.className =
                                        'badge badge-light-warning badge-sm ms-2';
                            ...
                            if (footer) {
                                ...
                                // Response Time
                                const responseTimeSpan = footer.querySelector(
                    ...
                    if (fullResponse) {
                        addMonitorLog('   Partial response received: ' + fullResponse.length +
                            ' chars', 'debug');
                    ...
                    if (data.message_id) {
                        ...
                        if (assistantBubble) {
                            ...
                            if (headerDiv && !headerDiv.querySelector('.badge-warning')) {
                                ...
                                errorBadge.title = 'This message contains an error explanation';
                                errorBadge.innerHTML =
                                    '<i class="ki-duotone ki-information-5 fs-7"><span class="path1"></span><span class="path2"></span><span class="path3"></span></i> Error Message';
                                const timeSpan = headerDiv.querySelector('.text-gray-500.fs-8');
                                if (timeSpan) {
                                    headerDiv.insertBefore(errorBadge, timeSpan);
                                } else {
                            ...
                            if (contentDiv && !contentDiv.parentElement.querySelector(
                                    '.retry-error-btn')) {
                                ...
                                retryBtn.onclick = function() {
                                    retryErrorMessage(data.message_id);
                                };
                                retryBtn.innerHTML =
                                    '<i class="ki-duotone ki-arrows-circle fs-6"><span class="path1"></span><span class="path2"></span></i> Retry with Higher Token Limit';
                        ...
                        const assistantMessageDiv = messagesContainer.querySelector(
                            '[data-message-id="' + assistantMessageId + '"]');
        ...
        stopBtn?.addEventListener('click', async () => {
            if (eventSource) {
                ...
                if (stoppedBeforeFirstChunk) {
                    ...
                    const allUserBubbles = Array.from(messagesContainer.querySelectorAll(
                        '.message-bubble.justify-content-end'));
                    const userBubble = allUserBubbles[allUserBubbles.length -
                        1]; // Get the last one
                    if (userBubble) {
                        userBubble.remove();
                    }
                    ...
                    toastr.warning(
                        'Stream stopped. User message not saved. Partial response will disappear on refresh.'
                    );
                }
        ...
        const clearConversation = async () => {
            ...
            if (result.isConfirmed) {
                ...
                window.location.href =
                    '{{ route('admin.llm.quick-chat.delete', ['sessionId' => $session?->id ?? 0]) }}';
            }
        };
        ...
        const renderExistingMessages = () => {
            ...
            const messageContents = messagesContainer.querySelectorAll(
            ...
            messageContents.forEach(contentDiv => {
                ...
                if (typeof marked !== 'undefined' && rawContent) {
                    try {
                        ...
                        contentDiv.innerHTML = rawContent.replace(/</g, '&lt;').replace(/>/g,
                            '&gt;').replace(/\n/g, '<br>');
                    }
                }
            });
